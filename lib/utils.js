...
utils.setHttpAgent = function setHttpAgent({
  ...
  if (bypassProxy) {
    ...
    if (sslExceptions) {
      const orig = config.httpOptions.agent.createConnection;
      config.httpOptions.agent.createConnection = (options, cb) => {
        options.checkServerIdentity = (hostname, cert) => {
          if (hostname in sslExceptions) {
            return undefined;
          }
          return tls.checkServerIdentity(hostname, cert);
        };
        return orig.apply(config.httpOptions.agent, [options, cb]);
      };
    }
  }

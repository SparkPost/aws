...
module.exports = ({
  ...
  if (sqsEndpoint) {
    endpointForSQS = {
      endpoint: new AWS.Endpoint(`${sqsProtocol}://${sqsEndpoint}`)
    };
  }

  if (s3Endpoint) {
    endpointForS3 = {
      endpoint: new AWS.Endpoint(`${s3Protocol}://${s3Endpoint}`)
    };
  }
  ...
  function getQueueURL(name) {
    const sqsPrefix = sqsEndpoint
      ? `${sqsProtocol}://${sqsEndpoint}`
      : `${sqsProtocol}://sqs.${AWS.config.region}.amazonaws.com`;

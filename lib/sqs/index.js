...
module.exports = ({
  ...
  if (s3Endpoint) {
    s3ep = { endpoint: new AWS.Endpoint(s3Endpoint), s3ForcePathStyle };
  }
  ...
  const S3 = setHttpAgent({ ...clientConfig, ...s3ep }, bypassProxy, AWS.S3);
  ...
  return {
     ...
     * @param {number}        maxS3Attempts maximum attempts to upload to S3 bucket
     */
    extendedSend: ({
      queueName,
      payload,
        ...
        compress(payload, { level })
          ...
          .then(async(payload) => {
            if (computeMessageSize(payload, attrs) > MAX_MSG_SIZE) {
              for (s3Attempt; s3Attempt < maxS3Attempts; s3Attempt++) {
                try {
                  ...
                  const s3Key = `/${prefix}/${uuid()}.json.gz`;
                  ...
                  await Promise.delay(Math.random() * 10);
                }
              }
            }
          ...
          .then((sqsResponse) => ({
            ...
            extended: !!key,
            key,
            s3UploadAttempts: key ? s3Attempt + 1 : 0
          }))
      );
    },
